const { getFileHandle, loadFileHandleAsDataUrl, fileExists } =
...
class TextureSetTab extends FileTab {
	...
	async setup() {
		...
		try {
			...
			this.textureImages = await this.loadImages(
				this.fileData['minecraft:texture_set']
			)
	...
	async loadTextureImages(textureName) {
		...
		const found = await Promise.all(
			extPriority.map(async (ext) => {
				const texturePath = join(imgPath, `/${textureName}.${ext}`)

				if (await fileExists(texturePath)) {
					return [
						ext,
						await loadFileHandleAsDataUrl(
							await getFileHandle(texturePath)
						),
					]
				}
	...
	receiveCanvas(canvas, imgSrc, layerPosition) {
		...
		if (width < 1 || height < 1) {
			throw Error('Failed getting image size from source')
		}
		...
		for (let itr = 0; itr < data.length; itr += 4) {
			...
			data[itr + this.METALNESS_INDEX] =
				layerPosition === this.METALNESS_INDEX
					? data[itr + this.METALNESS_INDEX]
					: data[currentIdx]
			data[itr + this.EMISSIVENESS_INDEX] =
				layerPosition === this.EMISSIVENESS_INDEX
					? data[itr + this.EMISSIVENESS_INDEX]
					: data[currentIdx]
			data[itr + this.ROUGHNESS_INDEX] =
				layerPosition === this.ROUGHNESS_INDEX
					? data[itr + this.ROUGHNESS_INDEX]
					: data[currentIdx]
		}
	...
	static parseColor(value) {
		...
		return `rgba(${Math.max(0, Math.min(255, r))}, ${Math.max(
			0,
			Math.min(255, g)
		)}, ${Math.max(0, Math.min(255, b))}, ${Math.max(0, Math.min(1, a))})`
	}
}

console.log(ThreePreviewTab)
...
try {
	registerPreview({
		...
		isFor: (tab) =>
			tab
				.getFile()
				.then((file) => file.name.endsWith('.texture_set.json')),
		trigger: (tab) => {
			tab.fileHandle
				.getFile()
				.then((file) => console.log('preview %O', file))
			openTab(TextureSetTab, true)
		},
	})
} catch (err) {
